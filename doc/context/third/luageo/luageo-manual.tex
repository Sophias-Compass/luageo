...
To draw a complete globe either start by filling a circle with the ocean color or finish by drawing a horizon circle. You can do both, as in the complete example below code. This code produces the large globe on this manual's first page.
...
However, notice that the country boundaries are visible but inconsistient. This happens because the boundaries between countries are drawn twice, once for each country.
...
The \type{globetrot} command may be removed if a better solution is found.
